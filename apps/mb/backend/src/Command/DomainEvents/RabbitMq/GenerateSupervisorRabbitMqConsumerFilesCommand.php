...
namespace PruebaTecnicaPh\Apps\Mb\Backend\Command\DomainEvents\RabbitMq;

use PruebaTecnicaPh\Shared\Domain\Bus\Event\DomainEventSubscriber;
use PruebaTecnicaPh\Shared\Infrastructure\Bus\Event\DomainEventSubscriberLocator;
...
{
    protected static $defaultName = 'pruebatecnicaph:domain-events:rabbitmq:generate-supervisor-files';
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        each($this->configCreate($path), $this->locator->all());
    }

    private function configCreate(string $path): callable
    {
    ...
    {
        return <<<EOF
[program:pruebatecnicaph_{queue_name}]
command      = {path}/apps/mb/backend/bin/console pruebatecnicaph:domain-events:rabbitmq:consume --env=prod {queue_name} {events_to_process}
