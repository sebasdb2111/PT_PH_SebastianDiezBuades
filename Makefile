...
test:
	@docker exec pruebatecnicaph-php_ddd_skeleton-php make run-tests

run-tests:
	mkdir -p build/test_results/phpunit
	./vendor/bin/phpunit --exclude-group='disabled' --log-junit build/test_results/phpunit/junit.xml tests
	./vendor/bin/behat -p mb_backend --format=progress -v
...
start-local:
	symfony serve --dir=apps/mb/backend/public --port=8030 -d --no-tls --force-php-discovery
...
stop-local:
	symfony server:stop --dir=apps/mb/backend/public
